...
SOURCE_SKILL = "ci-failure-analyzer"
...
def github_actions_evidence(log_text: str) -> dict:
    workflow_refs = sorted(set(normalize_rel(item) for item in re.findall(r"(\.github/workflows/[\w./\\-]+\.(?:yml|yaml))", log_text)))
    reusable_refs = sorted(set(re.findall(r"(?:uses:\s*|workflow_call[:\s]+)([^\n\r]+)", log_text, flags=re.IGNORECASE)))
    commands = sorted(set(re.findall(r"::(group|endgroup|error|warning|notice)(?:\s|::)", log_text, flags=re.IGNORECASE)))
    return {
        "debug_logging": {
            "runner_debug": "ACTIONS_RUNNER_DEBUG" in log_text,
            "step_debug": "ACTIONS_STEP_DEBUG" in log_text,
        },
        "workflow_commands": commands,
        "step_summary_present": "GITHUB_STEP_SUMMARY" in log_text,
        "workflow_references": workflow_refs,
        "reusable_workflow_references": reusable_refs,
        "artifact_attestation_mentioned": "attestation" in log_text.lower(),
        "diagnostic_gaps": [
            gap
            for gap, missing in (
                ("runner_debug_not_enabled", "ACTIONS_RUNNER_DEBUG" not in log_text),
                ("step_debug_not_enabled", "ACTIONS_STEP_DEBUG" not in log_text),
            )
            if missing
        ],
    }
...
def run_ci_failure_analyzer(repo_root: Path, request: dict, paths, output_path: Path) -> int:
    ...
    actions_evidence = github_actions_evidence(log_text)
    failure = {
        "id": "ci-failure-001",
        ...
        "github_actions_evidence": actions_evidence,
        "source_skill": SOURCE_SKILL,
    }
    ...
    output = {"artifact_type": "ci-failure-analyzer-output", "generated_at": utc_now(), "artifacts": {"ci_failure_report": str(paths.ci_failure_report), "diagnostic_findings": str(paths.diagnostic_findings)}, "failure_type": failure_type, "github_actions_signal_count": len([value for value in actions_evidence.values() if value])}
    write_json(output_path, output)
    print(output_path)
    return 0
