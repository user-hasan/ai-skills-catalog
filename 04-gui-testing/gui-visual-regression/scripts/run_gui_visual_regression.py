...
SOURCE_SKILL = "gui-visual-regression"
GUI_FAILURE_TYPES = {
    "widget failure",
    "event loop failure",
...
def run_gui_visual_regression(repo_root: Path, request: dict, paths, output_path: Path) -> int:
    baseline_raw = str(request.get("baseline_image", "")).strip()
    current_raw = str(request.get("current_image", "")).strip()
    ...
    if baseline is not None and current is not None and baseline.exists() and current.exists():
    ...
    comparison.update({"id": "visual-comparison-001", "baseline_image": str(baseline) if baseline is not None else "", "current_image": str(current) if current is not None else ""})
